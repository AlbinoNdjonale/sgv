...
class MultProcessing(QObject):
    ...
    def set_data(self):
        products = self.database["produto"].all(order_by = "nome")
    ...
    def set_log(self):
        logs = self.database["log"].all(order_by = "-date", where = {"type": {"eq": "'produto'"}})
    ...
    def to_filter(
        ...
        estoque    : QLineEdit,
        prazo      : QLineEdit,
        fornecedor : QLineEdit,
        tipo       : QLineEdit,
        timer      : QTimer
    ):
        
        def check(product): 
            if (id.text().strip() == str(product["id"]) or not id.text().strip())\
            and nome.text().strip().lower() in product["nome"].lower()\
            and (preco.text().strip() == str(product["preco"]) or not preco.text().strip())\
            and (estoque.text().strip() == str(product["estoque"]) or not estoque.text().strip())\
            and prazo.text().strip() in product["prazo"]\
            and fornecedor.text().strip().lower() in product["fornecedor"].lower()\
            and tipo.text().strip().lower() in product["tipo"].lower():
                return True
        ...
        products = [
            ...
            for product in self.database["produto"].all("nome")
            if check(product)
        ]
        ...
        if save:
            if deletes:     
                ...
                for delete in deletes:
                    save_log(
                        "produto",
                        self.database,
                        f"Produto deletado com sucesso "
                        f"(id do produto = {delete['Id']}, id do utilizador = {vendedor})"
                    )
            ...
            for change in changes:
                self.database["produto"].update(
                    {
                        "nome"      : change["Nome"],
                        "preco"     : change["Preço"].replace(".00 KZ", "").replace(" ", ""),
                        "estoque"   : change["Estoque"],
                        "prazo"     : change["Prazo"],
                        "fornecedor": change["Fornecedor"],
                        "tipo"      : change["Tipo"]
                    },
                    {"id": {"eq": change["Id"]}}
                )
                ...
                save_log(
                    "produto",
                    self.database,
                    f"Produto altertado com sucesso "
                    f"(id do produto = {change['Id']}, id do utilizador = {vendedor})"
                )
    ...
    def register(self, master, data, vendedor):
        ...
        if register:
            last_product = self.database["produto"].insert(data)
            
            QMessageBox.information(master, "Aviso", "Produto adicionado com sucesso.")
        
            save_log(
                "produto",
                self.database,
                f"Produto adicionado com sucesso "
                f"(id do produto = {last_product['id']}, id do utilizador = {vendedor})"
            ) 
...
class Produto(QHBoxLayout):
    def __init__(self, master, vendedor: dict, database: QBuilder, thread: QThread):
        ...
        lbl_visulize_sale = QLabel("Visualizar Produtos")
        ...
        self.lbl_n_products = QLabel(f"Total de Produtos: 0")
    ...
    def remove_to_table_product(self, row, _):
        self.lbl_n_products.setText(f"Total de Produtos: {len(self.table.rows)}")
